\chapter{Finding shortest path vectors in SpiNNaker's network}
	...
	In this chapter tackles the problem of finding shortest path vectors in hexagonal
	torus topologies. Shortest path vectors are used by many routing
	...
	\section{Shortest path vectors}
		...
		compute a \emph{shortest path vector} between the endpoints of a route and
		use this to generate a path through the network. A shortest path vector
		between two nodes is a vector, $\mathbf{v} = (v_1, v_2, v_3, \ldots)$,
		whose magnitude, $\| \mathbf{v} \| = \lvert v_1 \rvert + \lvert v_2 \rvert
	...
	\section{Related work}
		...
		\subsection{Hexagonal mesh networks}
			...
			\end{equation*}
			%
			For example, given the vector $(3, 2, 1)$ from `A' to `B' is minimised as
			follows:
			%
			\begin{align*}
		...
		\subsection{Hexagonal torus networks}
			...
			\subsubsection{XYZ-Protocol}
				...
				just one for destinations in quadrant 1 or 4 and no more than three in
				quadrant 2 or 3. Though the XYZ-protocol can be computed more cheaply
				than the INSEE method, it does not produce valid shortest path vectors
				for hexagonal torus topologies with aspect ratios other than $1:1$.
	...
	\section{Generating all shortest path vectors}
			...
			vector $(0, H, 0)$ straight-forwardly wraps once around the Y axis
			arriving back to the node it started from. The vector $(1,1,1)$ also
			returns to its starting point as described in
			appendix~\ref{app:minimal-hex-coordinates}. As a consequence $(0,H,0) -
			H\cdot(1,1,1) = (-H, 0, -H)$ must also be a vector which leads back where
			it started. Adding this vector to a shortest path vector of the form $(x,
			0, z)$ results in a new shortest path vector if $x\ge H$ and $z\le0$.
